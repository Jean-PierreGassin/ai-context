...
validate_retired_manifest_entry() {
  local manifest_path="$1" expected_sha="$2" relative_path="$3"

  if [[ ! "$expected_sha" =~ ^[0-9a-f]{40}$ || -z "$relative_path" ]]; then
    record_failure "invalid retired managed file entry in $manifest_path"
    return 1
  fi
...
reject_symlinked_managed_parent() {
  local target_path="$1" target_root="$2" display_path="$3" parent_path
...
remove_empty_managed_parents() {
  local parent_path="$1" target_root="$2"
...
remove_retired_managed_files() {
  local manifest_path="$1" target_root="$2" display_prefix="$3"
  ...
  while IFS=$'\t' read -r expected_sha relative_path; do
    ...
    validate_retired_manifest_entry "$manifest_path" "$expected_sha" "$relative_path" || continue

    target_path="$target_root/$relative_path"
    display_path="${display_prefix}${relative_path}"

    if reject_symlinked_managed_parent "$target_path" "$target_root" "$display_path"; then continue; fi
    ...
    rm -f -- "$target_path"
    remove_empty_managed_parents "$(dirname "$target_path")" "$target_root"
  done <"$manifest_path"
}

build_snapshot_payload() {
  local payload_root="$1" manifest_path="$2" snapshot_payload_root="$3"
  local expected_sha relative_path placeholder_path

  rm -rf -- "$snapshot_payload_root"
  cp -R "$payload_root" "$snapshot_payload_root"
  [[ -f "$manifest_path" ]] || return 0
